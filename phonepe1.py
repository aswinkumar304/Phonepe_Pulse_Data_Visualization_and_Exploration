...
def animate_all_amount():
    # URL of GeoJSON data for Indian states
    url = "https://gist.githubusercontent.com/jbrobst/56c13bbbf9d97d187fea01ca62ea5112/raw/e388c4cae20aa53cb5090210a42ebb9b765c0a36/india_states.geojson"
    # Fetch GeoJSON data from the URL
    response =requests.get(url)
    # Load GeoJSON data into a Python dictionary
    ...
    frames = []
    # Loop through unique years and quarters in the Map_user DataFrame
    for year in Map_user["Years"].unique():
        for quarter in Aggre_trans["Quarter"].unique():
            # Select rows from Aggre_trans based on year and quarter
            at1 = Aggre_trans[(Aggre_trans["Years"]==year)&(Aggre_trans["Quarter"]==quarter)]
            # Extract relevant columns: States and Transaction_amount
            atf1 = at1[["States","Transaction_amount"]]
            # Sort the DataFrame based on States
            atf1 = atf1.sort_values(by="States")
            # Add Years and Quarter columns to the DataFrame
            atf1["Years"]=year
            atf1["Quarter"]=quarter
            # Append the DataFrame to the frames list
            frames.append(atf1)
    merged_df = pd.concat(frames)# Concatenate all frames into a single DataFrame
...
# bar chart for transaction type and count
def payment_count():
...
def animate_all_count():
    ...
    fig_tra.update_layout(title_font={"size":25})
    return st.plotly_chart(fig_tra)
# bar chart for transaction type and amount
def payment_amount():
...
# bar char for payment count per year
...
# bar chart for payment type and amount
...
# bar chart for districts and register user
...
# bar chart for district and transacction amount
...
tab1, tab2 = st.tabs(["**EXPLORE THE PHONEPE DATA VISUALIZATION**","**DIFFERENT FACTS AND FIGURES**"])
